...
ActiveRecord::Schema[8.1].define(version: 20_260_816_150_200) do
...
unique: true
    t.index %w[user_id year period_type], name: 'index_digests_on_user_year_period_type_yearly', unique: true,
where: '((month IS NULL) AND (period_type <> 2))'
    t.index %w[user_id year week period_type], name: 'index_digests_on_user_year_week_weekly', unique: true,
where: '(period_type = 2)'
  ...
  create_table 'achievement_progresses', force: :cascade do |t|
    ...
    t.index %w[user_id achievement_key], name: 'index_achievement_progresses_on_user_id_and_achievement_key',
unique: true
  end
  ...
  create_table 'flights', force: :cascade do |t|
    ...
    t.datetime 'updated_at', null: false
    t.bigint 'user_id', null: false
    t.index %w[user_id departure_time], name: 'index_flights_on_user_id_and_departure_time'
    t.index %w[user_id external_id], name: 'index_flights_on_user_id_and_external_id', unique: true
  ...
  create_table 'geofence_events', force: :cascade do |t|
    ...
    t.datetime 'updated_at', null: false
    t.bigint 'user_id', null: false
    t.index %w[area_id occurred_at], name: 'index_geofence_events_on_area_id_and_occurred_at',
order: { occurred_at: :desc }
    t.index ['area_id'], name: 'index_geofence_events_on_area_id'
    t.index ['lonlat'], name: 'index_geofence_events_on_lonlat', using: :gist
    t.index %w[user_id occurred_at], name: 'index_geofence_events_on_user_id_and_occurred_at',
order: { occurred_at: :desc }
  ...
  create_table 'notes', force: :cascade do |t|
    ...
    t.datetime 'updated_at', null: false
    t.bigint 'user_id', null: false
    t.index 'attachable_type, attachable_id, ((noted_at)::date)', name: 'index_notes_on_attachable_and_noted_date',
unique: true, where: '(attachable_id IS NOT NULL)'
    t.index %w[attachable_type attachable_id], name: 'index_notes_on_attachable_type_and_attachable_id'
    t.index ['lonlat'], name: 'index_notes_on_lonlat', using: :gist
    t.index %w[user_id noted_at], name: 'index_notes_on_user_id_and_noted_at'
  ...
  create_table 'places', force: :cascade do |t|
    ...
    t.index "user_id, ((geodata ->> 'external_place_id'::text))", name: 'idx_places_user_external_place_id',
unique: true, where: "((geodata ->> 'external_place_id'::text) IS NOT NULL)"
...
where: "((raw_data_archived = false) AND (raw_data <> '{}'::jsonb))"
    t.index %w[user_id timestamp lonlat], name: 'index_points_on_user_id_timestamp_lonlat', unique: true
    t.index ['user_id'], name: 'idx_points_user_id_legacy_tracker',
where: "((tracker_id)::text = ANY (ARRAY[('google-maps-timeline-export'::character varying)::text, ('google-maps-phone-timeline-export'::character varying)::text]))"
  ...
  create_table 'shared_links', id: :uuid, default: -> { 'gen_random_uuid()' }, force: :cascade do |t|
    ...
    t.index %w[resource_type resource_id], name: 'index_shared_links_on_resource_type_and_resource_id',
...
where: '(privacy_radius_meters IS NOT NULL)'
  ...
  create_table 'track_segments', force: :cascade do |t|
    ...
    t.index %w[track_id start_at], name: 'idx_track_segments_track_start_at_unique', unique: true,
where: '(start_at IS NOT NULL)'
    t.index %w[track_id start_index end_index], name: 'index_track_segments_on_track_and_indices'
    t.index %w[track_id start_index], name: 'idx_track_segments_track_start_index_unique', unique: true
  ...
  create_table 'tracks', force: :cascade do |t|
    ...
    t.datetime 'updated_at', null: false
    t.bigint 'user_id', null: false
    t.index "user_id, COALESCE(tracker_id, ''::character varying), start_at, end_at",
            name: 'index_tracks_on_user_tracker_start_end_unique', unique: true
    ...
    t.index %w[user_id tracker_id end_at], name: 'idx_tracks_user_tracker_end_at'
  ...
  create_table 'user_achievements', force: :cascade do |t|
    ...
    t.datetime 'updated_at', null: false
    t.bigint 'user_id', null: false
    t.index %w[user_id achievement_key], name: 'index_user_achievements_on_user_id_and_achievement_key', unique: true
  end
  ...
  create_table 'user_devices', force: :cascade do |t|
    ...
    t.datetime 'updated_at', null: false
    t.bigint 'user_id', null: false
    t.index %w[user_id device_id], name: 'index_user_devices_on_user_id_and_device_id', unique: true
...
'minutes_between_routes' => '60' }
    ...
    t.index %w[provider uid], name: 'index_users_on_provider_and_uid_present', unique: true,
where: '((provider IS NOT NULL) AND (uid IS NOT NULL))'
    t.index ['reset_password_token'], name: 'index_users_on_reset_password_token', unique: true
    t.index ['signup_variant'], name: 'index_users_on_signup_variant_reverse_trial',
where: "((signup_variant)::text = 'reverse_trial'::text)"
  ...
  create_table 'visits', force: :cascade do |t|
    ...
    t.index %w[user_id started_at place_id], name: 'idx_visits_user_started_at_place_unique', unique: true
  ...
  create_table 'webhook_deliveries', force: :cascade do |t|
    ...
    t.index %w[webhook_id created_at], name: 'index_webhook_deliveries_on_webhook_id_and_created_at',
